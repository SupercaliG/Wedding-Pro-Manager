...
interface AdminDropRequestsClientProps {
  escalatedRequests: DropRequest[];
  pendingRequests: DropRequest[];
  resolvedRequests: DropRequest[];
}

export default function AdminDropRequestsClient({
  escalatedRequests,
  pendingRequests,
  resolvedRequests
}: AdminDropRequestsClientProps) {
  ...
  return (
    <div>
      ...
      <Tabs defaultValue="escalated" onValueChange={setActiveTab}>
          ...
          {escalatedRequests.length > 0 ? (
            <div>
              <p className="text-gray-600 mb-4">
                These requests have been escalated due to the 24-hour SLA. They require immediate attention.
              </p>
              {escalatedRequests.map(request => renderDropRequestCard(request, true))}
            </div>
          ) : (
        ...
        <TabsContent value="pending">
          {pendingRequests.length > 0 ? (
            <div>
              <p className="text-gray-600 mb-4">
                These requests are still within the 24-hour SLA window and can be handled by managers, but admins can also approve or reject them.
              </p>
